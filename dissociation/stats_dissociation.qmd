...
x.labels <- paste0(x.labels, "<br><span style = 'font-size:20pt'>n=", n.tots$ntot[match(names(x.labels), n.tots$Condition)], "</span>")
names(x.labels) <- unique(proportions.df$Condition)
...
ggplot(proportions.df, aes(x = Condition, y = proportion)) +
    ...
    stat_pvalue_manual(
        stats %>%
            filter(p.signif != "ns") %>%
            mutate(y.position = 1.03 + 1:length(group1) * 0.08),
        label = "p={signif(p, 2)}",
        tip.length = 0.01
    ) +
    theme_classic() +
    ...
    ylab("% elongated gastruloids") +
    scale_x_discrete(labels = x.labels) +
    xlab("")
# For reproducible jitter:
set.seed(28)
ggsave(file.path(output.dir, "prop.elong.png"), width = 5, height = 5)
set.seed(28)
ggsave(file.path(output.dir, "prop.elong.pdf"), width = 5, height = 5)
```
