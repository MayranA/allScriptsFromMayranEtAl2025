...
new.cluster.name <- c("Neural Tube", "Som. Mes.", "Post. PSM",
                      "NMP 1", "Caudal Mes.", "Prim. Streak",
                      "Dermomyotome", "Neuron Progen. 1", "Epiblast",
                      "NMP 2", "Neuron Progen. 2", "Sclerotome",
                      "Ant. PSM", "ESCs", "Endoderm 2",
                      "Neuron Precursor", "Pharyngeal Mes.",
                      "Ant. Prim. Streak", "Endoderm 1",
                      "Pluripotent", "Unknown",
                      "Endothelial", "Surface Ecto.")
...
params$current.fig <- SwitchFigure(Fig = "S1", Panel = "Reviewer")
genelist <- c("Lmo1", "Kdr", "Tbx6", "Msgn1", "Mesp1",
              "Nkx2-5", "Hand2", "Gata6", "Pax2", "Osr1", "Lhx1",
              "Chrd", "Noto", "Nodal", "Bmp4", "Tbx1", "Hand1", "Foxf1",
              "T", "Sox2")
for(gene in genelist){
  g <- featurePlotMayran("combined.seurat", gene, params)
  print(g)
}
params$current.fig <- SwitchFigure(Fig = "S4", Panel = "Snai1.WT")
genelist <- c("Snai1")
for(gene in genelist){
  g <- featurePlotMayran("combined.seurat", gene, params)
  print(g)
...
Here we identify the markers between the Prim. Streak and Ant. Prim. Streak fates

```{r}
marker.APS.vs.PS <- FindMarkers(combined.seurat,
                                ident.1 = "Prim. Streak", 
                                ident.2 = "Ant. Prim. Streak",
                                logfc.threshold = 0.3,
                                min.diff.pct = 0.2
                                )
write.csv(marker.APS.vs.PS, file = file.path(output.directory, "markers.APS.vs.PS.csv"))
```
